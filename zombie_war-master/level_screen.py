...
class level_screen(pygame.sprite.Sprite):

    def __init__(self, name):
        self.name = name

        selected_data = game.Game(self.name)
        game_data = selected_data.loadGame()

        self.strength = game_data.get(self.name).get('strength')
        self.r_rate = game_data.get(self.name).get('r_rate')
        self.speed = game_data.get(self.name).get('speed')
        ...
        while True :
            for event in pygame.event.get():
                ...
                if(event.type == pygame.QUIT):
                    ...
                    gdisplay.fill(white)
                    this_list = selected_data.housePositions()


                    for i in this_list:
                        self.current_house = i

                        self.temp = house_sprite.house_sprite(self.current_house.x_coor, self.current_house.y_coor)
                        self.temp.update(self.current_house.player_control)
                        self.print_population()
                        allsprites.add(self.temp)
    ...
    def clicked(self):
        ...
        if self.current_house.x_coor + 100 > mouse[0] > self.current_house.x_coor and self.current_house.y_coor + 200 > mouse[1] > self.current_house.y_coor:
            if click == (1,0,0):


                self.house_list.append(self.current_house.connections)
                self.other_list.append(self.current_house.house_name)
                self.check_others()
    ...
    def check_others(self):
        ...
        for i in self.house_list:
            if(self.current_house.house_name in i):
                # print(self.current_house.house_name)

                # print((self.current_house.house_name, j))
                self.house_list = []
                self.other_list = []
    ...
    def print_population(self):
        ...
        some_text= font.render(str(self.current_house.player_pop), 1, black)
        gdisplay.blit(some_text, (self.current_house.x_coor + 60, self.current_house.y_coor - 30))

    # def the_right_house(self):
    #     if():
